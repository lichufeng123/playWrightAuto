...
import-manual-xmind  人工测试点 XMind + PRD docx -> 04_reviewed 测试点 JSON
...
如果某些需求不适合让 AI 从 PRD 解析测试点，你可以自己先写好测试点 XMind，再用单独命令导入。
...
```text
末级叶子节点 = 测试点
叶子节点的上级路径 = modulePath
如果叶子节点有备注，会作为 source 写入 JSON
根节点如果叫“测试点总览/测试点清单/人工测试点”，会被视为容器，不进入 modulePath
```

执行命令：

```powershell
python scripts/ai_testcase_pipeline.py import-manual-xmind --prd "prd/V3.4/V3.4需求PRD.docx" --xmind "prd/V3.4/人工测试点.xmind" --run-dir "output/ai-testcase-generation/V3.4需求PRD-会员订阅付费系统"
```

防呆规则：

```text
--prd 是必填项，不传 PRD 会直接报错。
脚本会检查 PRD 文件名、人工 XMind 文件名、run-dir 是否明显匹配。
如果 PRD 是“会员订阅付费系统”，XMind/run-dir 是“智能分镜脚本”，脚本会停止导入，避免复制错 PRD。
如果确实需要跨需求复用 PRD，可显式增加 --allow-prd-mismatch。
后续 testpoints-to-cases、review-cases、revise-cases 也会读取 run_metadata.json 做同样校验，避免继续使用历史遗留的错误 PRD。
```
...
```powershell
python scripts/ai_testcase_pipeline.py testpoints-to-cases --run-dir "output/ai-testcase-generation/V3.4需求PRD-会员订阅付费系统"
python scripts/ai_testcase_pipeline.py review-cases --run-dir "output/ai-testcase-generation/V3.4需求PRD-会员订阅付费系统"
python scripts/ai_testcase_pipeline.py revise-cases --run-dir "output/ai-testcase-generation/V3.4需求PRD-会员订阅付费系统"
```
...
每批 AI 原始返回会单独保存，便于排查模型返回 JSON 格式错误：

```text
05_test_cases.batch_01.raw.txt
05_test_cases.batch_02.raw.txt
...
```

如果出现 `Expecting ',' delimiter` 这类 JSON 解析错误，脚本会提示具体失败批次和对应 raw 文件路径。

脚本会先尝试自动修复模型返回的 JSON：

```text
1. 先用本地规则修复常见尾逗号
2. 如果仍失败，再调用模型把该批 raw 返回修成合法 JSON
3. 修复后的内容会保存为 *.repaired.txt
4. 修复后仍失败才终止并报错
```
...
Review 报告中需要人工判断的章节会使用 Markdown 表格，并包含 `审核意见` 列，默认值为 `待人工审核`。你可以后续手动改成 `修改`、`忽略`、`已处理`、`需产品确认` 等。

报告不会再输出 `优先级调整建议` 章节，因为当前测试用例模板没有优先级字段。

Review 报告会在生成后再做一次去重整理。同一个用例/测试点的同一个核心问题只保留一处，避免在多个章节里重复让你审核。

`审核意见` 常用写法：

```text
修改：第六步会按修复建议修改用例
忽略：第六步不会按该建议修改，也不会生成补充用例
暂定：第六步不会编造规则，会在相关用例备注中写清问题说明和待确认原因
需产品确认：同暂定，保留问题说明，等待确认
已处理：按已处理状态生成修订版
```
...
补充用例会自动归一化到测试点使用的模块分类口径。例如模型返回：
...
```text
会员订阅付费系统/套餐定制留资/留资资料
```

`07_test_cases_revised.xlsx` 中，`REVIEW-xxx` 这类 Review 补充用例会用浅黄色高亮，方便你一眼看出哪些是第六步新增的。
...
`08_revision_summary.md` 会总结本次根据 Review 的处理结果，包括：

```text
哪些用例修改了
哪些补充了
哪些因为审核意见为“忽略”没有处理
哪些因为审核意见为“暂定/需产品确认”只在备注中保留问题说明
哪些仍需要人工确认
```
