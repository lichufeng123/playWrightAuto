...
本文帮助新同事快速理解本项目中 AI 员工自动化的 Page Object 定义与用例设计方式，并补充说明与其相邻的 AI 群组自动化现状。

## pages/agent.page.ts：核心元素与方法
- 入口与就绪：`waitForReady()` 确认跳转到 `/aichat` 并等待左侧员工列表可见；`waitForAgentListReady()` 通过“侧栏可见 + 添加按钮可见 + 搜索框可用 + 加载中消失”判断列表进入稳定态。
...
虽然这份文档主要讲 AI 员工，但当前仓库里 AI 群组已经和员工模块走了相似的自动化模式。
...
- 4 个策略群组的数据驱动 prompt 管理
- 自愈式补群组：缺失时自动从“添加AI群组”弹窗补齐
- 串行 `batch messaging`
